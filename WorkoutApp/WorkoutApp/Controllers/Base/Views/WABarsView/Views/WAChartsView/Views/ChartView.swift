...
//  WorkoutApp
//
//  Created by Всеволод Буртик on 03.09.2024.
//
...
private extension ChartView {
    ...
    func drawChart(with data: [WAChartsView.Data], topChartOffset: Int) {
        ...
        let valuePoits = data.enumerated().map({CGPoint(x: CGFloat($0), y: CGFloat($1.value))})
        let chartHeight = bounds.height / CGFloat(maxValue + topChartOffset)
        
        let points = valuePoits.map{
            let x = bounds.width / CGFloat(valuePoits.count - 1) * $0.x
